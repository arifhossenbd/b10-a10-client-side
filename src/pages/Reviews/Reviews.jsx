...
const Reviews = ({ reviews }) => {
  ...
  return (
    <div
      className={`card bg-base-100 flex flex-col shadow-xl rounded-none ${transition} hover:shadow-2xl font-orbitron cursor-pointer h-full`}
    >
      ...
      <div className={`card-body p-4 flex flex-col flex-grow ${transition}`}>
        <h2 className="card-title md:text-xl font-bold overflow-hidden text-ellipsis line-clamp-2 text-wrap">{title}</h2>
        <p className="text-gray-600 overflow-hidden text-ellipsis line-clamp-2">{genres}</p>
